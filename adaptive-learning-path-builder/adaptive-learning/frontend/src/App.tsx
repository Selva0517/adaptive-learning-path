...
function getNodeColor(type: NodeType) {
  if (type === 'start') return { bg: '#10b981', border: '#059669', text: '#e04c4c' };
  if (type === 'assessment') return { bg: '#3b82f6', border: '#2563eb', text: '#fff' };
  if (type === 'unit') return { bg: '#8b5cf6', border: '#7c3aed', text: '#fff' };
  return { bg: '#ffffff', border: '#475569', text: '#fff' };
}

function getNodeHeight(node: LPNode): number {
  return node.type === 'start' || node.type === 'end' ? 52 : NODE_HEIGHT;
}

function nodeCenter(node: LPNode): Position {
  return { x: node.position.x + NODE_WIDTH / 2, y: node.position.y + getNodeHeight(node) / 2 };
}

function nodeBottom(node: LPNode): Position {
  return { x: node.position.x + NODE_WIDTH / 2, y: node.position.y + getNodeHeight(node) };
}

function nodeTop(node: LPNode): Position {
...
function EdgeLayer({ nodes, edges, selectedEdgeId, onSelectEdge, pendingConnect }: EdgeLayerProps) {
  ...
  // Compute a bounding box large enough to contain all nodes + generous padding
  const bounds = useMemo(() => {
    if (nodes.length === 0) return { minX: 0, minY: 0, width: 4000, height: 4000 };
  ...
  return (
    <svg
      style={{ position: 'absolute', top: bounds.minY, left: bounds.minX, overflow: 'visible', pointerEvents: 'none' }}
      width={bounds.width}
    ...
    >
      ...
      {edges.map(edge => {
        ...
        // Connect from bottom of source to top of target for clean vertical flows
        const s = nodeBottom(src);
        const t = nodeTop(tgt);
        // Offset SVG coordinates by bounds origin
        const sx = s.x - bounds.minX;
        const sy = s.y - bounds.minY;
        const tx = t.x - bounds.minX;
        const ty = t.y - bounds.minY;
        const selected = edge.id === selectedEdgeId;
        const dy = Math.abs(ty - sy);
        const curveStrength = Math.max(40, Math.min(120, dy * 0.45));
        const cp1 = { x: sx, y: sy + curveStrength };
        const cp2 = { x: tx, y: ty - curveStrength };
      ...
      {pendingConnect && (() => {
        ...
        const sx = s.x - bounds.minX;
        const sy = s.y - bounds.minY;
        const tx = pendingConnect.toPos.x - bounds.minX;
        const ty = pendingConnect.toPos.y - bounds.minY;
        return (
...
function LeftPanel({ components, loading, onDragItem }: LeftPanelProps) {
  ...
  return (
    <div style={{ width: 240, background: '#ffffff', borderRight: '1px solid #1e2130', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
      <div style={{ padding: '14px 14px 10px' }}>
        ...
        <input
          ...
          style={{ width: '100%', background: '#ffffff', border: '1px solid #2d3148', borderRadius: 6, padding: '6px 10px', color: '#e2e8f0', fontSize: 12, outline: 'none', marginBottom: 8 }}
        />
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 10px 10px' }}>
        {loading ? (
          ...
          <div
            ...
            style={{ background: '#ffffff', border: `1px solid ${item.type === 'assessment' ? '#1e3a8a' : '#3b1d7a'}`,
      ...
      {/* How it works */}
      <div style={{ margin: 10, background: '#ffffff', borderRadius: 8, padding: '10px 12px', border: '1px solid #1e2130' }}>
...
function PropertiesPanel({ selectionType, selectedNode, selectedEdge, allNodes, onUpdateNode, onUpdateEdge, onDeleteSelected }: PropertiesPanelProps) {
  if (!selectionType) {
    return (
      <div style={{ width: 260, background: '#ffffff', borderLeft: '1px solid #1e2130', padding: 16, color: '#475569', fontSize: 12 }}>
  ...
  return (
    <div style={{ width: 260, background: '#ffffff', borderLeft: '1px solid #1e2130', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
...
function NodeProperties({ node, onUpdate }: { node: LPNode; onUpdate: (n: LPNode) => void }) {
  const field = (label: string, value: string | number, onChange: (v: string) => void, type = 'text') => (
    <div style={{ marginBottom: 14 }}>
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)}
        style={{ width: '100%', background: '#ffffff', border: '1px solid #2d3148', borderRadius: 6, padding: '7px 10px', color: '#e2e8f0', fontSize: 12, outline: 'none' }} />
    </div>
  );
  ...
  return (
    <>
      ...
      <div style={{ marginTop: 6, padding: '8px 10px', background: '#ffffff', borderRadius: 6, fontSize: 10, color: '#475569' }}>
        ID: {node.id}
      </div>
    </>
  );
...
function RuleEditor({ rule, onUpdate, onRemove, srcNodeType }: { rule: ConditionRule; onUpdate: (p: Partial<ConditionRule>) => void; onRemove: () => void; srcNodeType?: NodeType }) {
  ...
  return (
    <div style={{ background: '#ffffff', border: '1px solid #2d3148', borderRadius: 7, padding: '10px 10px 8px', marginBottom: 8 }}>
      ...
      <select value={rule.metric} onChange={e => onUpdate({ metric: e.target.value as Metric })}
        style={{ width: '100%', background: '#ffffff', border: '1px solid #2d3148', borderRadius: 5, padding: '5px 8px', color: '#e2e8f0', fontSize: 11, marginBottom: 7, outline: 'none' }}>
      ...
      {(rule.metric === 'completion' || rule.metric === 'passed') && (
        <select value={rule.value === true ? 'true' : 'false'} onChange={e => onUpdate({ value: e.target.value === 'true', operator: 'eq' })}
          style={{ width: '100%', background: '#ffffff', border: '1px solid #2d3148', borderRadius: 5, padding: '5px 8px', color: '#e2e8f0', fontSize: 11, outline: 'none' }}>
      ...
      {rule.metric === 'score' && (
        <div style={{ display: 'flex', gap: 5 }}>
          <select value={rule.operator} onChange={e => onUpdate({ operator: e.target.value as Operator })}
            style={{ flex: 1, background: '#ffffff', border: '1px solid #2d3148', borderRadius: 5, padding: '5px 6px', color: '#e2e8f0', fontSize: 11, outline: 'none' }}>
          ...
          <input type="number" value={rule.value as number || 0} onChange={e => onUpdate({ value: parseInt(e.target.value) || 0 })}
            style={{ flex: 1, background: '#ffffff', border: '1px solid #2d3148', borderRadius: 5, padding: '5px 8px', color: '#e2e8f0', fontSize: 11, outline: 'none' }} />
        </div>
      )}
...
export default function App() {
  ...
  }, [connectFrom, edges, draggingNodeId]);
  ...
  return (
    <div style={{ height: '100vh',  display: 'flex', flexDirection: 'column', background: '#0f1117', fontFamily: 'Inter,system-ui,sans-serif' }}>
      {/* TOPBAR */}
      <div style={{ height: 52, background: '#ffffff', borderBottom: '1px solid #1e2130', display: 'flex', alignItems: 'center', padding: '0 18px', gap: 16, flexShrink: 0, zIndex: 100 }}>
        <div>
        ...
        <div style={{ flex: 1, display: 'flex', justifyContent: 'center' }}>
          <input value={pathName} onChange={e => setPathName(e.target.value)}
            style={{ background: 'transparent', border: '1px solid #ffffff', borderRadius: 7, padding: '5px 14px', color: '#e2e8f0', fontSize: 13, fontWeight: 600, textAlign: 'center', outline: 'none', width: 280 }} />
        </div>
        ...
        <button onClick={handleLoadPaths} style={{ background: '#ffffff', border: '1px solid #2d3148', borderRadius: 7, padding: '6px 14px', color: '#94a3b8', fontSize: 12, cursor: 'pointer' }}>
          Load Path
        </button>
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={handleSaveDraft} disabled={saveStatus === 'saving'}
            style={{ background: '#ffffff', border: '1px solid #2d3148', borderRadius: 7, padding: '7px 16px', color: '#94a3b8', fontSize: 12, cursor: 'pointer', fontWeight: 600 }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div
          ref={canvasRef}
          style={{ flex: 1, position: 'relative', overflow: 'hidden', background: '#ffffff', cursor: connectFrom ? 'crosshair' : isPanning ? 'grabbing' : 'default' }}
      ...
      {/* Load paths modal */}
      {showSavedPaths && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(255, 255, 255, 0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}
          onClick={() => setShowSavedPaths(false)}>
          <div style={{ background: '#fdfdfd', border: '1px solid #2d3148', borderRadius: 12, padding: 24, width: 480, maxHeight: '70vh', overflow: 'auto' }}
            ...
            {savedPaths.length === 0 ? (
              ...
              <div key={lp.id} style={{ background: '#ffffff', borderRadius: 8, padding: '12px 14px', marginBottom: 8, cursor: 'pointer', border: '1px solid #2d3148' }}
